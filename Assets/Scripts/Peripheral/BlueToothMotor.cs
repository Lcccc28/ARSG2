...
//	}
	...
	void Update () {
		...
		if (timeout != 0 && timeout <= Time.realtimeSinceStartup) {
			...
			case States.Scan:
				Time.timeScale = 0;
				Alert.Show("未连接到蓝牙设备，扫描中...", null, false);
				// TODO 这里暂停事件
				BluetoothLEHardwareInterface.ScanForPeripheralsWithServices (new string[]{BluetoothLEHardwareInterface.FullUUID(broadcastUUID)}, (_address, _name) => {
					if (!illegalDevices.Contains (_address)) {
						...
						Alert.Show("正在连接到设备："+_name, null, false);
...
#endif
						Alert.Show("连接成功！", (_value) => { SetState (States.Subscribe, 1f); });
					}
				}, (_address) => {
	...
	void StartProcess ()
	{
		Alert.Show("蓝牙初始化中，如未开启蓝牙设置将初始化失败", null, false);
		BluetoothLEHardwareInterface.Initialize (true, false, () => {
			SetState (States.Scan, 0.2f);
		}, (error) => {
			// Console.Write ("蓝牙初始化失败: " + error + "，等待重试");
			Alert.Show("初始化失败，请开启蓝牙后重试", (_value) => {
